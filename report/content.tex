...
\begin{figure}[bth]
	\myfloatalign
	\subfloat[Traditional database.]
	{\includegraphics[width=0.4\linewidth]{gfx/cenVsDec}} \quad
	\subfloat[Blockchain.]
	{\includegraphics[width=0.4\linewidth]{gfx/cenVsDec2}} \\
	\caption{Centralized approach compared to a decentralized approach (blockchain).}
...
\begin{table}[bth]
	...
	\begin{tabularx}{\textwidth}{p{4cm}p{4cm}|p{4cm}p{4cm}}
		...
		Node types per peering node & uniform distrib. [3,5]  & Computing demand $d_i$ & uniform distrib. [1,3] \\
		Node capacity $\mu_{max}$ & uniform distrib. [5,100] & Mean arrival rate $\lambda$ & 1 request per second \\
	\end{tabularx}
...
Therefore, thanks to the presented algorithm, the InPs define their product costs regarding the current demands and resources utilization, which provides real-time and on-demand values. These costs are further used to quote prices for the requested virtual node while taking into account the desired profit margins. In future research, this cost estimation can be performed based on many other parameters, which will enable InPs to have a better market positioning.
  ...
  \qquad \mu = \frac{\mu_u + d_i}{\mu_{max}} \qquad \mu_{max} \in [200,300], \ \mu_u \in [0, \mu_{max}] , \ d_i \in [1, 10]   
  \end{equation}
...
For this reason, after examining consensus mechanism that provides medium-high transaction rates in a scalable and untrusted model, we have ended up with the \textbf{Proof of Elapsed Time (PoET)} protocol. Other consensus algorithms, such as Proof-of-Stake, Byzantine Fault Tolerance or Proof-of-Authority \citep{baliga2017understanding}, where excluded either due to the integration complexity into our current Ethereum platform or since they sacrifice the trust of our system (not hybrid).
...
\begin{table}[htbp]
	...
	\begin{tabular}{m{2cm} m{13.5cm}}
		...
		$\mu_{max}$ & variable $\in [5,100]$ expressing the maximum capacity of a resource type in substrate node $N_s^{*}$ \\
		$\mu_u$ & variable $\in [0,\mu_{max}]$ expressing the used capacity of a resource type in substrate node $N_s^{*}$ \\
		$\alpha^{h}_{s^{*}}(N_i)$ & dynamic hourly unitary cost for virtual node $N_i \in N^{R}$ \\
...
For \ref{fig:acceptance_rate} and \ref{fig:pricing}, 3 InPs.
For \ref{fig:acceptance_rate_3D} average values, not shown the CI. $L_1$ and $\lambda = 5$

\begin{figure}[bth]
	\centering
	\includegraphics[scale=1]{gfx/ev_acceptance_rate3D.png}    
  	\caption{Acceptance rate $\varphi$ with different number of InPs.}
