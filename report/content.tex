...
Contemplating once again the flowchart in Figure \ref{fig:bcFlowchart} and observing the last mentioned requirements, we foresee that blockchain could be a suitable solution for our VNE scenario suffering from LID. However, the significant choice between blockchain types still needs to be argued. Obviously, the service negotiation between InPs and SPs is performed in a private environment, which automatically removes the permissionless and permissioned public blockchain options. Hence, the decision should be taken between a private or a hybrid blockchain.
...
\begin{itemize}
    \item \textbf{Easy to use and learn:} A low effort in setting the platform is essential. Furthemore the smart contracts should be written or derived from common languages in the world of programming.
    \item \textbf{Support and documentation:} A consolidated platform that is not in its early stages and has achieved a high-maturity level, is extremely needed. An active community of developers behind the project is also fundamental, e.g. with a considerable number of Github stars and forks.
    \item \textbf{Blockhain type:} Though it could be desirable that the main network is permissioned and hybrid or private, currently, in open source code platforms, the blockchain types can be modified.
\end{itemize}
...
Smart contracts step by step code
\begin{lstlisting}
contract Coin {

		// Declares a state variable that stores the balances for each possible address.
    mapping (address => uint) balances;
	
		// Constructor whose code is run only when the contract is created.
    function Coin() public {
    }
		// Function that sends 'amount' to the receiver address.
    function send(address receiver, uint amount) returns(bool isSent) {
...
In the following code extracted from \footnote{\url{https://solidity.readthedocs.io/en/develop/introduction-to-smart-contracts.html}}.
