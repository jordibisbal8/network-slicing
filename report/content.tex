...
The blockchain is a disruptive technology that enables the management of distributed workflows by automating the execution of complex processes. This system enables mistrusted entities to query and alter information without trusting a middle party, while providing a secure data storage in a decentralized scenario. In this thesis, we focus on an IT supply chain example, where multiple actors negotiate a tenancy agreement, network virtualization. This process consists of service providers (SPs) requesting the embedding of virtual networks across multiple infrastructure providers (InPs). However, these InPs are not willing to disclose detailed information about their internal network topologies and resources, which directly hampers the efficiency of the service negotiation. After observing how the blockchain can face the challenges encountered in supply chain processes and reviewing the previous work on centralized and decentralized virtual network embedding (VNE) systems, we reason why the blockchain is an appropriate workaround for the described scenario. In order to enhance the VNE throughput, a brokerless system, that uses the blockchain, smart contracts and a VN partitioning algorithm based on the Vickrey auction model is designed and subsequently implemented. Finally, we further investigate the feasibility of our solution, by first analyzing the behavior of the introduced auction model in adverse conditions and by second evaluating the blockchain performance given different consensus models.
...
On balance, we do not claim that our approach is the only way of performing VN partitioning using the blockchain technology, however,  thanks to smart contract's flexibility, this can serve as a starting point for more upcoming investigations.
