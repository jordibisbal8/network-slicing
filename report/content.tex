...
New transactions are stored in the last block of the blockchain, and a new block is mined every ten minutes. Over time, this creates an ever-growing chain of blocks, which are constantly updated. Thus the name: blockchain. Additionally, a complete history of the transactions is kept, so everyone can verify the last money movements. For instance, a blockchain can be compared to an endless domino game, where all the pieces are placed in vertical one after the other. Each of these pieces references a block, and if one block is not placed correctly (block's error), all the subsequent ones will be affected. Therefore, miner's task is not simple and requires computational power. Figure \ref{fig:Bitcoin mining} shows a chain of blocks, where each block contains a:

\begin{figure}[t]
  	\includegraphics[width=1\linewidth]{gfx/mining}    
  	\caption{Bitcoin Mining \citep{buterin2014next}}
...
\begin{itemize}
	...
	\item Timestamp.
	\item Nonce.
	\item Hash reference to the previous block. 
	\item List of all the transactions that have been created since the previous block.
\end{itemize}
...
\begin{figure}[bth]
  \includegraphics[width=1\linewidth]{gfx/ethereumAccounts}    
...
In the following, we discuss related work on blockchain in supply chain management, virtual network embedding across multiple InPs and auction mechanisms.
...
However, normally most companies store and maintain its data into their company-specific or public systems (AWS\footnote{https://aws.amazon.com/}). Accordingly, each enterprise creates its own copy of the product data, using their own protocols or procedures.  As a consequence, lots of information asymmetries are found, which ends up destroying the benefits of knowledge or data sharing. In \citep{lee1992managing} inadequate definition of customer services, poor coordination or organizational barriers are named to be some of the pitfalls that customer and providers suffer from. Nevertheless, many other research \citep{fiala2005information},\citep{li2008confidentiality} have also investigated the causes of information asymmetries through the supply chain.

START TO EXPLAIN APPPLICATIONS in BLOCKCHAIN ASSET tracking, multiple ...

\cite{mattila2016product}.
...
1. Invite People to the private BC. Users fixed amount of Ether ( Proof of Stake).
