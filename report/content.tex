...
\begin{itemize}
	...
	\item \textbf{Service Providers: (SPs)} Service providers lease virtual resources from infrastructure providers to create its customized virtual network. All this without the need to purchase physical network equipment.
		\item \textbf{Virtual network providers: (VNPs)} Are the brokers, which can optionally serve as intermediaries between the InPs and the service providers. VNE consists in mapping the VN request onto multiple InPs
\end{itemize}
...
The last two functions are usually performed by the VNPs and InPs respectively. Furthemore, in such scenarios exists two types of communication:
...
In the last years, many investigations in network virtualization, and specifically in VNE, have been conducted by different authors \citep{houidi2011virtual}, \citep{zhu2008cabernet}, \citep{chowdhury2009virtual}. Among these, it is important to highlight topics such as the trade-offs between single \citep{chowdhury2009virtual}, \citep{houidi2008distributed} and multi-provider VNE \citep{dietrich2017multi}, or the different techniques and algorithms used for achieving VN efficiency, which decompose VNE into two tasks: VN partitioning and VN segment mapping \citep{fischer2013virtual}. Each of these studies proposes new interesting aspects or features compared to the prior research. Nevertheless, in this paper, we will not contrast the different architectures (e.g. with or without VNP) or discuss the various VN partitioning and mapping algorithms. The main goal of this work will be to explore an existing scenario, in particular, frameworks for multi-provider VNE applications that suffer from the limited information disclosure problem \citep{dietrich2017multi}, \citep{zaheer2010multi}, \citep{esposito2013general}, \citep{chowdhury2010polyvine}.

\subsection{Multi-provider Virtual Network Embedding with Limited Information Disclosure}
 
Multi-provider virtual network embedding with LID (Figure \ref{fig:multiprov}.a), performed by a service provider, a third-party or VNP and multiple InPs (section \ref{networkvirtualization}), consists of the allocation of virtual resources in a constrained and limited scenario. The main cause of this problem is that InPs are not willing to broadcast their resources information (e.g: nodes availability, cost) or their network topology to the outside world \citep{dietrich2015multi}. And if that was not enough, this information is crucial so that VNPs can perform the partitioning of resources, in order to obtain a fair negotiation between the SPs and InPs.  

In the last mentioned research, these LID partitioning problems are approached accessing public information that is not considered confidential by the InPs:
...
Furthermore, in \citep{dietrich2015multi}, a VNE framework is implemented, which performs subsequently the following tasks: Firstly, the above-mentioned resource information is sent from the InPs to the VNPs, who registers and stores it. Secondly, the service provider request is matched with this collected data in the VNP. Finally, the corresponding partitioning and mapping algorithms are applied (not discussed in this paper). Nevertheless, they are proved to provide embedding costs not much higher than in an ideal case scenario (InPs announcing all required information). Despite the positive results, this scenario suffers mainly from scalability, since everything relies on the virtual network provider. In addition, it assumes that InPs (if they do so) will be constantly advertising their updated data to the VNP, which can lead to undesired costs.

Therefore, other approaches where the actors benefit from a more scalable and decentralized service negotiation need to be further investigated.
...
In addition, already deployed decentralized approaches regarding VN embedding \citep{esposito2013general}, \citep{chowdhury2010polyvine} will also be in our spotlight. \citep{esposito2013general} super example with consensus protocol! GOOD images
SUPPRESS VNP
